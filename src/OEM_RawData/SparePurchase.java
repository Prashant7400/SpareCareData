...
//		String filetype = "pdf";
		...
		prefs.put("download.default_directory", "C:\\Users\\Admin\\Desktop\\TATA CVBU & PCBU OUTPUT DATA\\SparePurchase");
		...
		FileInputStream file = new FileInputStream("C:\\Users\\Admin\\Desktop\\TATA CVBU &  PCBU INPUT DATA\\SparePurchaseData.xlsx");
		...
		for (int i = 1; i <= rowcount; i++) {
			...
			String status = celldata.getCell(2).getStringCellValue();
			String transactiondate = celldata.getCell(3).getStringCellValue();
//			String division = celldata.getCell(4).getStringCellValue();
//			String from = celldata.getCell(5).getStringCellValue();
//			String to = celldata.getCell(6).getStringCellValue();
//
...
//			driver.findElement(By.xpath("(//a[text()=\"Receiving\"])[1]")).click();
			...
			WebElement st = driver.findElement(By.xpath("//td[@aria-roledescription=\"Status\"]"));
			
			Actions act = new Actions(driver);
			act.click(st).build().perform();
			                                                 
			
			WebElement sta =  driver.findElement(By.xpath("//input[@name=\"Availability_Code\"]"));
			sta.sendKeys(status);
			
			// Transaction date value
			
	       WebElement trdate = driver.findElement(By.xpath("//td[@aria-roledescription=\"Transaction Date\"]"));
			
			act.click(trdate).build().perform();
			
			WebElement tradate = driver.findElement(By.xpath("//input[@id=\"1_Transaction_Date\"]"));
			tradate.sendKeys(transactiondate);
			
			WebElement clo = driver.findElement(By.xpath("//a[@class=\"siebui-task-assist-close\"]"));
			js.executeScript("arguments[0].click();", clo);
			...
			wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//div[text()=\"SAP Invoice #\"]")));
